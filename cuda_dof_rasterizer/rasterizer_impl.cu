...
__global__ void identifyTileRanges(int L, uint64_t* point_list_keys, uint2* ranges)
{
	...
	// current tile index of duplicated 3D GS
...
CudaRasterizer::BinningState CudaRasterizer::BinningState::fromChunk(char*& chunk, size_t P)
{
	...
	// sorted results will be in binning.point_list_keys and binning.point_list
...
int CudaRasterizer::Rasterizer::forward(
	// here geometryBuffer should be passed with a function
	// with input as size_t and return the char*
	...
	bool debug)
{
	...
	// firstly, assign the space with chunk_size to chunkptr (pointing at start now)
	char* chunkptr = geometryBuffer(chunk_size);
	// then, give each parameters in GeometryState the corresponding space (pre-assigned before) by fromChunk
	...
	// buffer is defined as this
	// so it just assign the GPU space?
	...
	// geomState.means2D, float2*
	// geomState.depths, float*
	// geomState.rgb, float*
